...
\toprule
\textbf{Variable} & \textbf{Categorías} & \textbf{Dummy} \\
\midrule
\placeholder{ECG\_LOCAL} & \placeholder{Ant, Inf, Lat} & \placeholder{LOC\_ant, LOC\_inf, LOC\_lat} \\
\placeholder{ECG\_RITMO} & \placeholder{Sin, FA, Fl} & \placeholder{RITMO\_sin, RITMO\_fa} \\
\bottomrule
\end{tabular}
}
\end{table}
...
\toprule
\textbf{Variable} & \textbf{Categorías (orden)} & \textbf{Valores} \\
\midrule
KILLIP & I $<$ II $<$ III $<$ IV & 1, 2, 3, 4 \\
\placeholder{TABAQ.} & \placeholder{Nunca $<$ Ex $<$ Activo} & \placeholder{0, 1, 2} \\
\bottomrule
\end{tabular}
}
\end{table}
